...
def aggregate_dataset(
    ...
    multirun: bool = False,
    leave: bool = False,
    filter_datasets: Optional[tuple[str]] = (
        "sber_bank",
        "twitch",
        "yoochoose",
        "xwines",
    ...
    if not leave and len(filter_datasets) == 0:
        leave = True
    filter_mod = filterfalse if leave else filter
    for dataset in filter_mod(
        lambda name: name.stem in filter_datasets, path.iterdir()
    ):
        for filename in dataset.iterdir():
            # print(filename.stem)
            if (
                multirun
                and "nfold" not in filename.stem
                or not multirun
                and "nfold" in filename.stem
            ):
                continue
            if multirun:
            ...
            if filename.stem in ("results", "results_wasOptimized_True"):
                file_path = filename
                break
            if filename.stem == "results_wasOptimized_False":
                file_path = filename
...
def aggregate_methods(
    metric_path: Path,
    filter_datasets: tuple[str],
    methods_with_subfolders: tuple[str] = ("implicit", "recbole", "replay", "msrec"),
    stop_methods: tuple[str] = ("replay", "user_knn", "sasrec"),
    multirun: bool = False,
) -> pd.DataFrame:
    ...
    for method in filterfalse(
        lambda name: name.stem in stop_methods, metric_path.iterdir()
    ):
        if method.stem in methods_with_subfolders:
            for sub_method in method.iterdir():
                res_df_method = aggregate_dataset(
                    sub_method, multirun, filter_datasets=filter_datasets
                )
            ...
            res_df_method = aggregate_dataset(
                method, multirun, filter_datasets=filter_datasets
            )
...
def filter_data(
    ...
    if drop_minor_methods:
        val_cnts = df["Method"].value_counts()
        max_instances_method = val_cnts[0]
        saved_methods = val_cnts[val_cnts == max_instances_method].index
        df = df[df["Method"].isin(saved_methods)]
