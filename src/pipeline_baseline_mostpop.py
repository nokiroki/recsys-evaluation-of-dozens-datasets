...
class BaselineMostpopRunner(BaseRunner):
    ...
    def run(cfg: DictConfig) -> None:
        ...
        dataset = ClassicDataset()
        dataset.prepare(cfg_data)
        data_train, data_test = BaselineMostpopRunner._load_or_split_data(
            cfg_data, dataset_folder, dataset
        )
        shape = BaselineMostpopRunner._get_shape(data_train)
        ...
        metrics_df = BaselineMostpopRunner._calculate_metrics(
